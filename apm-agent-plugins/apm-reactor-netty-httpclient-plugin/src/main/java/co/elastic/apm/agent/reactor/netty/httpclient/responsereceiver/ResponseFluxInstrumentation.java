...
public class ResponseFluxInstrumentation extends AbstractResponseReceiverInstrumentation {
    ...
    public static class AdviceClass {
        ...
        public static <T extends HttpClient.ResponseReceiver<?>> Flux<?> onExit(
            ...
            @Advice.Argument(0) BiFunction<? super HttpClientResponse, ? super ByteBufFlux, ? extends Publisher<T>> receiveFunction,
            @Advice.Return Flux<?> originalReturn) {

            try {
                if (modifiedReceiver != null) {
                    return modifiedReceiver.response(receiveFunction);
                } else {
